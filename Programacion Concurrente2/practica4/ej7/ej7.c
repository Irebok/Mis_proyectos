...
printf("\nNumero de saltos máximos seguidos a lectores: %i",max_saltos);
...
struct thread_info *tinfo = arg;
	...
	printf("[Lector %i]-> Intentando leer...\n",tinfo->n);
		...
		if(numero_lectores == 1){sem_wait(&se_papel);
					 sem_wait(&se_memoria_escribir);

						while(numero_escritores>0 &&  numero_saltos != max_saltos){
						sem_post(&se_papel);
					  	sem_post(&se_memoria_escribir);				
						printf("\n");
						numero_saltos++;
					       printf("Se ha saltado %i veces el primer lector",numero_saltos);			
					  	sem_wait(&se_papel);
					  	sem_wait(&se_memoria_escribir);
						}
					printf("\nsale while");						
...
struct thread_info *tinfo = arg;
	...
	sem_wait(&se_memoria_escribir);
	printf("\nNumero de saltos:%i",numero_saltos);
