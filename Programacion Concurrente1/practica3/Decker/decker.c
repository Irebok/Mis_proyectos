...
int id = shmget(key, 3*sizeof(int), IPC_CREAT | 0600);
	if(id == -1){
...
buffer[2] = turno;
	...
	printf("Intentando entrar en la seccion critica\n");
		...
		getchar();
		printf("Yendo a la entrada\n");
		
			while(buffer[2] != yo_seccion){
			printf("Aun no es mi turno de salir\n");
			getchar();
			}
		}
	}
